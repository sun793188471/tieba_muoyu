...
**版本**: v2.0.5 | **协议**: MIT | **适用**: 百度贴吧 (`tieba.baidu.com`)

### v2.0.5 补丁

- 修复从首页切换到贴吧 Sheet 后仍无图片：隐藏渲染页的 `src` 是贴吧通用占位图，现优先读取 `data-original` 中的真实缩略图
- 继续保留 `bpic` 作为查看大图时的第一高清候选

### v2.0.4 补丁

- 修复 v2.0.3 在部分油猴隔离环境读取 `window.frameElement` 时中断初始化，导致 `R`、设置入口和全部功能无响应的问题
- 内部动态图片渲染页改用专用 URL 参数识别，不再依赖可能受沙箱限制的窗口属性

### v2.0.3 补丁

- 修复从贴吧首页点击底部贴吧 Sheet 时，静态响应未执行贴吧媒体初始化脚本而导致帖子缩略图全部丢失的问题
- 静态响应无图时改为读取同源贴吧页面渲染后的列表 DOM，并继续优先保留 `bpic` 原图候选
- `E` 为隐藏状态时，有图帖子显示 `[图片]` 占位；按一次 `E` 可恢复缩略图

### v2.0.2 补丁

- `R` 现在是脚本内容与展示托管的总开关；关闭时不隐藏、不删除、不折叠或改写贴吧原生内容
- `E` / `Q` / `D` 仅在 Excel 模式开启后生效，关闭 `R` 时会完整恢复原生页面
- 修复进入具体贴吧后 Excel 帖子列表缺少图片的问题
- 吧页图片点击查看时优先加载贴吧提供的 `bpic` 原图，失败后再回退其他候选和缩略图
- 兼容贴吧旧 GBK 编码的吧名链接，避免 Sheet 标签出现乱码
- 帖子 Sheet 顶栏新增“↗ 打开原贴”，可在新标签页打开对应贴吧帖子

## ✨ v2.0.0 更新重点

- Excel 模式内点击帖子后直接创建帖子 Sheet，不再跳出伪装页面
- 修复链接包裹图片、帖子正文图片丢失和楼中楼被采集逻辑误删的问题
- 表格继续显示轻量缩略图，点击查看时优先加载 `imgsa` / `imgsrc` 高清原图，失败自动回退
- 修复隐藏图片等快捷键状态覆盖基础设置、扩展坞首次打不开设置、导入导出首次无效等问题
- 表格、关键字和黑名单管理按纯文本处理站点与用户输入，降低 HTML 注入风险
- 完整审计 16 个功能开关和 3 个高级参数，增加数值范围校验、配置彻底重置和自动化回归测试
...
1. 安装上述浏览器扩展
2. 打开 [`tiebamuoyu.js`](https://raw.githubusercontent.com/sun793188471/tieba_muoyu/main/tiebamuoyu.js)
3. 扩展弹出安装页面后确认安装
4. 访问 [百度贴吧](https://tieba.baidu.com/)，脚本自动生效

已安装 v1 的用户也可以重新打开上述链接覆盖安装；v2 内置更新地址，后续可由脚本管理器检查更新。
...
|--------|------|:----------:|
| `R` | 切换 Excel 伪装模式 | ✅ |
| `Q` | Excel 模式内切换隐藏/显示头像 | ✅ |
| `E` | Excel 模式内切换隐藏/显示图片 | ✅ |
| `D` | Excel 模式内切换暗黑模式 | ✅ |

> **托管边界**：只有开启 `R` 后，脚本的内容过滤和展示设置才会生效；关闭 `R` 会恢复贴吧原生页面。`Q` / `E` / `D` 的状态会保存，供下次进入 Excel 模式时恢复。
...
- **原贴入口**：查看帖子 Sheet 时，顶栏显示“↗ 打开原贴”，可随时在新标签页打开对应贴吧原帖
...
勾选后隐藏帖子列表、帖子详情和楼中楼中的用户头像；取消勾选则恢复显示，减少视觉干扰。
...
全屏图片查看器，功能包括：
- **高清原图优先**：从贴吧图片文件名生成 `imgsa` / `imgsrc` 原图候选，加载失败自动回退到表格缩略图
...
| 设置项 | 默认值 | 有效范围 | 说明 |
|--------|:------:|:--------:|------|
| 图片最大宽度 | 250px | 80–1600px | 帖内图片最大显示宽度 |
| 折叠高度阈值 | 200px | 80–2000px | 楼中楼超过该高度时折叠 |
| 全局字体大小 | 14px | 10–32px | 页面全局字体大小 |
...
`npm test` 覆盖设置面板、全部设置项、Excel 内联帖子、图片解析、高清候选回退、内容过滤、主题、自动翻页和输入安全等场景。
...
贴吧可能对非浏览器请求返回 403；此时可先在浏览器保存公开帖 HTML，再将文件路径作为 `tests/live-tieba-smoke.js` 的第二个参数执行。
